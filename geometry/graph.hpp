...
namespace geometry {
	...
	template<graph_types _Graph_type,
		bool _Random_access_vertices, bool _Binary_search_vertices, bool _Random_access_out_edges,
		typename _Vertex_id, 
		typename _Vertex_property,
		typename _Edge_property  = no_property,
		typename _Graph_property = no_property,
		bool     _Labeled        = !std::is_integral_v<_Vertex_id> >
	class adjacency_list {
	public:
		static constexpr graph_types type = _Graph_type;
		static constexpr bool labeled = _Labeled;
		static_assert( labeled || std::is_integral_v<_Vertex_id> );
		///@note
		///                    binary_search    no_binary_search
		/// random_access      std::vector<..>  std::vector<...>
		/// no_random_access   std::map<..,..>  std::list<...>
		static constexpr bool random_access_vertices = _Random_access_vertices;
		static constexpr bool binary_search_vertices = _Binary_search_vertices;
		static constexpr bool random_access_out_edges = _Random_access_out_edges;
		...
		using vertex_id       = _Vertex_id;
		using vertex_property = _Vertex_property;
		using edge_property   = _Edge_property;
		using graph_property  = _Graph_property;
		

		template<geometry::graph_types _Which_graph_type>
		...
		template<>
		struct _Which_container<geometry::graph_types::undirected> {
			using vertex_descriptor = std::conditional_t<random_access_vertices, size_t, void*>;
			struct edge_descriptor { 
				vertex_descriptor source; std::conditional_t<random_access_out_edges, size_t, void*>
					impl/* = decltype(source.out_edges)::iterator{ target, edge_container::iterator }*/,
		...
		template<>
		struct _Which_container<geometry::graph_types::bidirected> {
			using vertex_descriptor = std::conditional_t<random_access_vertices, size_t, void*>;
			struct edge_descriptor { 
				vertex_descriptor source; std::conditional_t<random_access_out_edges, size_t, void*>
					impl/* = decltype(source.out_edges)::iterator{ target, edge_container::iterator }*/,
		...
		using container = _Which_container<_Graph_type>;
		...
		vertex_descriptor find(vertex_id id) {
			if constexpr (!labeled) {
				if constexpr (random_access_vertices)
		...
		std::pair<edge_descriptor, bool> _Insert_or_assign_edge(vertex_descriptor source, vertex_descriptor target, edge_property_any&&... prop) {
			...
			if constexpr (type == geometry::directed) {
				...
				if (result == (*this)[source].out_edges.end()) {
					...
					if constexpr (type == geometry::undirected) {
		...
		edge_descriptor find(vertex_id s_id, vertex_id t_id) {
		...
		void clear() noexcept {
			c.vertices.clear();
			if constexpr (type == undirected || type == bidirected) {
				c.edges.clear();
			}
		}
	...
	template<graph_types _Graph_type,
		...
		typename _Edge_property  = no_property,
		typename _Graph_property = no_property>
	using labeled_adjacency_list = adjacency_list<_Graph_type, _Random_access_vertices, _Binary_search_vertices, _Random_access_out_edges,
		_Vertex_id, _Vertex_property, _Edge_property, _Graph_property, true>;
		

#if 0
	template<bool Dt, bool Rv, bool Bv, bool Re, bool Be, typename Vi, typename Vp, typename Ep, typename Gp,
		typename dest_container>
		requires requires(dest_container& __c) { __c.push_back(typename geometric::adjacency_list<Dt, Rv, Bv, Re, Be, Vi, Vp, Ep, Gp>::vertex_descriptor{}); }
	size_t find_roots(geometric::adjacency_list<Dt, Rv, Bv, Re, Be, Vi, Vp, Ep, Gp>& g, dest_container& roots) {
		std::vector<bool> visited(g.vertices.size(), false);
		for (auto& vertexX : g.vertices) {
			for (auto& out_edgeX : vertexX.out_edges) {
				visited[ std::distance(g.vertices.begin(), g._Get_iter(g.vertices, out_edgeX.first)) ] = true;
			}
		}
		...
		for (auto viter = g.vertices.begin(); viter != g.vertices.end(); ++viter, ++i) {
			if (!visited[i]) {
				roots.push_back( g._Get_descriptor(g.vertices, viter) );
				++ct;
			}
		}
		return ct;
